...
def handle_batch_command(args: argparse.Namespace) -> int:
    ...
    for index, input_path in enumerate(input_paths, start=1):
        ...
        if not args.format:
            source_image, source_format = load_image(input_path)
            source_image.close()

        output_format = resolve_format("-", args.format, source_format)
        destination = build_batch_destination(input_path, output_dir, output_format)

        stderr(
            f"[{index}/{total}] {args.operation} {input_path} -> {destination}"
        )
        rendered = process_single(
            input_path,
            str(destination),
