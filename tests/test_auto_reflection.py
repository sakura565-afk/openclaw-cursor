...
class AutoReflectionTests(unittest.TestCase):
    ...
    def test_run_reflection_writes_learnings_and_skips_writes_on_dry_run(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertTrue((learnings / "reflections").exists())
            day_md = learnings / "reflections" / f"{auto_reflection.utc_now().date().isoformat()}.md"
            self.assertTrue(day_md.is_file())
            day_body = day_md.read_text(encoding="utf-8")
            self.assertIn("Session analysis", day_body)
            self.assertIn("Pattern detection", day_body)
            self.assertIn("Actionable recommendations", day_body)
            self.assertIn("Stats summary", day_body)
            json_runs = list((learnings / "reflections").glob("run_*.json"))
            self.assertEqual(len(json_runs), 1)
            payload = json.loads(json_runs[0].read_text(encoding="utf-8"))
