...
Provides ``analyze_recent_sessions()`` (log-focused scan under ``logs/`` by default),
``generate_reflection_report()`` (markdown), integration with
``src.self_improvement.auto_engine.AutoImprovementEngine`` when available, writes under
``.learnings/`` and ``memory/reflections/``, and optionally posts the summary (Telegram
or generic webhook).

Example crontab (daily at 09:00 UTC):

    0 9 * * * cd /path/to/repo && /usr/bin/python3 cursor-repo/scripts/auto_reflection.py
...
CORRECTION_HINTS = re.compile(
    r"(?i)(\bcorrected\b|\bfixed\b|\bresolved\b|\bworkaround\b|\bpatched\b|"
    r"\bupdated\s+to\b|\bchanged\s+from\b|\breran\b|\bretry\b|\binstead\b)"
)
DEFAULT_LOG_SESSION_GLOBS = ("logs/**/*.log", "logs/**/*.json")
...
class SessionAnalysis:
    """Structured view of recent logs plus optional auto-engine entries."""

    root: Path
    cutoff_utc: datetime
    started_at_utc: datetime
    globs_used: tuple[str, ...]
    session_files: list[str]
    insights: list[Insight]
    engine_entries: list[dict[str, Any]]
    error_patterns: list[tuple[str, int]]
    correction_patterns: list[tuple[str, int]]
...
def _repo_root_for_imports() -> Path:
    """Workspace root (parent of ``cursor-repo``)."""

    return Path(__file__).resolve().parents[2]


def fetch_auto_engine_entries(root: Path, *, since_days: int = 14) -> list[dict[str, Any]]:
    """Load recent JSON lines from ``AutoImprovementEngine`` logs under ``logs/``."""

    repo = _repo_root_for_imports()
    ...
    engine = AutoImprovementEngine(root_dir=root, log_dir=root / "logs")
    return list(engine.iter_log_entries(since_days=since_days))
...
def analyze_recent_sessions(
    ...
    *,
    ...
    include_auto_engine: bool = True,
    engine_since_days: int = 14,
) -> SessionAnalysis:
    """Scan recent session files (default: under ``logs/``) and extract patterns."""

    started = started_at or utc_now()
    effective_globs = tuple(globs) if globs is not None else DEFAULT_LOG_SESSION_GLOBS
    session_paths = iter_session_files(root, effective_globs, cutoff)

    insights: list[Insight] = []
    for sf in session_paths:
    ...
    engine_entries: list[dict[str, Any]] = []
    if include_auto_engine:
        engine_entries = fetch_auto_engine_entries(root, since_days=engine_since_days)

    err_counter: Counter[str] = Counter()
    corr_counter: Counter[str] = Counter()
    for ins in insights:
        key = normalize_insight_text(ins.text)[:200]
        if not key:
            continue
        if LESSON_HINTS.search(ins.text) or CORRECTION_HINTS.search(ins.text):
            corr_counter[key] += 1
        if ins.severity in {"error", "warning"} or (
            FAILURE_HINTS.search(ins.text) and ins.severity != "info"
        ):
            err_counter[key] += 1

    error_patterns = err_counter.most_common(25)
    correction_patterns = corr_counter.most_common(25)

    rel_sessions = [p.relative_to(root).as_posix() for p in session_paths]

    return SessionAnalysis(
        root=root,
        cutoff_utc=cutoff,
        started_at_utc=started,
        globs_used=effective_globs,
        session_files=rel_sessions,
        insights=insights,
...
def _format_patterns_and_engine(analysis: SessionAnalysis) -> str:
    lines: list[str] = []

    if analysis.error_patterns:
        lines.append("## Recurring error signals")
        lines.append("")
        for text, n in analysis.error_patterns:
            lines.append(f"- ({n}×) {text}")
        lines.append("")

    if analysis.correction_patterns:
        lines.append("## Corrections and lessons")
        lines.append("")
        for text, n in analysis.correction_patterns:
            lines.append(f"- ({n}×) {text}")
        lines.append("")

    if analysis.engine_entries:
        lines.append("## Auto-improvement engine (recent)")
        lines.append("")
        lines.append("| timestamp | category | action | outcome |")
        lines.append("| --- | --- | --- | --- |")
        for entry in analysis.engine_entries[-30:]:
            ts = str(entry.get("timestamp", ""))[:28]
            cat = str(entry.get("category", "")).replace("|", "\\|")
            act = str(entry.get("action", "")).replace("|", "\\|")[:80]
            out = str(entry.get("outcome", "")).replace("|", "\\|")
            lines.append(f"| {ts} | {cat} | {act} | {out} |")
        lines.append("")

    return "\n".join(lines).rstrip()


def generate_reflection_report(analysis: SessionAnalysis) -> str:
    """Render a markdown reflection including insights, patterns, and engine context."""

    top_sessions = analysis.session_files[:15]
    base = build_summary_markdown(
        analysis.started_at_utc,
        len(analysis.session_files),
        analysis.insights,
        top_sessions,
    )
    extra = _format_patterns_and_engine(analysis)
    if not extra:
        return base
    return base.rstrip() + "\n\n" + extra + "\n"


def analysis_to_jsonable(analysis: SessionAnalysis) -> dict[str, Any]:
    return {
        "root": str(analysis.root),
        "cutoff_utc": analysis.cutoff_utc.replace(microsecond=0).isoformat(),
        "started_at_utc": analysis.started_at_utc.replace(microsecond=0).isoformat(),
        "globs_used": list(analysis.globs_used),
        "session_files": analysis.session_files,
        "insights": [asdict(i) for i in analysis.insights],
        "engine_entries": analysis.engine_entries,
        "error_patterns": analysis.error_patterns,
        "correction_patterns": analysis.correction_patterns,
    }


def write_memory_reflections(root: Path, run: ReflectionRun, analysis: SessionAnalysis) -> tuple[Path, Path]:
    """Persist run artifacts under ``memory/reflections/``."""

    out_dir = root / "memory" / "reflections"
    out_dir.mkdir(parents=True, exist_ok=True)
    base = out_dir / f"run_{run.run_id}"
    md_path = base.with_suffix(".md")
    json_path = base.with_suffix(".json")

    md_lines = [
        f"# Reflection run {run.run_id}",
        "",
        f"- Analysis cutoff (UTC): `{analysis.cutoff_utc.isoformat()}`",
        f"- Globs: {', '.join(f'`{g}`' for g in analysis.globs_used)}",
        "",
    ...
    payload = {
        ...
        "reflection_run": asdict(run),
        "session_analysis": analysis_to_jsonable(analysis),
    }
    ...
    latest.write_text(
        json.dumps(
            {
                "reflection_md": md_path.relative_to(root).as_posix(),
                "reflection_json": json_path.relative_to(root).as_posix(),
...
def run_reflection(
    ...
    *,
    ...
    analysis = analyze_recent_sessions(
        root,
        cutoff=cutoff,
        globs=globs,
        started_at=started,
        include_auto_engine=True,
    )
    summary = generate_reflection_report(analysis)
    ...
    run = ReflectionRun(
        ...
        files_scanned=len(analysis.session_files),
        session_files=analysis.session_files,
    ...
    write_memory_reflections(root, run, analysis)

    state["last_run_utc"] = finished.replace(microsecond=0).isoformat().replace("+00:00", "Z")
    state["last_run_id"] = run_id
    state["last_insight_count"] = len(analysis.insights)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Analyze recent agent logs; refresh .learnings/ and memory/reflections/; optionally post a summary.",
    )
    parser.add_argument(
        "--root",
        type=Path,
